    ...
    $('#publish').click(function(){
        ...
        if (title.val().length < 1){
            if( $('#required-title-message').length == 0){
                $('#titlewrap').after('<label id="required-title-message">* Opportunity title is required</label>'); 
        ...
        if( !validateNumeric(value) && value.length > 0){
            if( $('#invalid-value-message').length == 0){
                $('#_wpcrm_opportunity-value').after('<label id="invalid-value-message">* Please enter a valid Value</label>'); 
